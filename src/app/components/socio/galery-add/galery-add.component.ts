...
import { Firestore, Timestamp, addDoc, collection, doc, getDocs, query, serverTimestamp, setDoc, updateDoc, where } from '@angular/fire/firestore';
...
export class GaleryAddComponent implements OnInit{
  ...
  async onSubmit(url:string) {
    try {
      ...
      // addDoc(queryRef3, data)
      await setDoc(doc(queryRef3, id), {
        title:this.formulario.get('title')!.value,
        description:this.formulario.get('description')!.value,
        timestamp: serverTimestamp(),
        name:this.id,
...
getImages() {
  const imagesRef = ref(this.storage, 'galery');
  listAll(imagesRef)
    .then(async response => {
      console.log(response);
      this.images = [];
...
getDownloadURL(imgRef: StorageReference) {
  getDownloadURL(imgRef)
    .then((url) => {
      ...
      this.onSubmit(url);
      // Aquí puedes utilizar la URL como desees, por ejemplo, mostrar la imagen en la página o almacenarla en la base de datos.
    })
    .catch((error) => {
